...
const pokemonsReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case DELETE_POKEMON:
      ...
      if (findPoke) {
        const filteredArr = state.pokemons.filter(
          (el) => el.id !== action.payload
        );
        const filteredBackup = state.backup.filter(
          (el) => el.id !== action.payload
        );
        return {
          ...state,
    ...
    case FILTER_BY_TYPE:
      ...
      return {
        ...state,
        pokemons: backupCopy.filter((pokemon) =>
          pokemon.types.some((el) => el.name === action.payload)
        ),
      };
    default:
      return { ...state };
